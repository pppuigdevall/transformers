...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        logger.debug(f"output_hidden_states = {output_hidden_states}")
        logger.debug(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            logger.debug(f"About to infer layer {i}")
            layer_outputs = layer_module(
                x=hidden_state, attn_mask=attn_mask, head_mask=head_mask[i], output_attentions=output_attentions
            )
            logger.debug(f"Done inference {i}")
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            logger.debug("About to do embeddings in line 570")
            inputs_embeds = self.embeddings(input_ids)  # (bs, seq_length, dim)
            logger.debug("Embeddings created successfully")
        logger.debug("STARTING TRANSFORMER INFERENCE")
